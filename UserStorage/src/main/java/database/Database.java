...
public class Database {
	...
	public void countUsers() {
		...
		try {
			...
			System.out.println("There are " + count + " users currently on the database");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void editUser(String first, String last, int id) {
		Statement stmt;
		try {
			stmt = getConnection().createStatement();
			String user = String.format("UPDATE USER SET first_name='%s', last_name='%s' WHERE id=%d",first,last,id );
			stmt.executeUpdate(user);
			System.out.println("User updated on database");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
